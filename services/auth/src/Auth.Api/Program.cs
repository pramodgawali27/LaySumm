...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1", new OpenApiInfo
    {
        Title = "Auth API",
        Version = "v1",
        Description = "OIDC integration and token exchange facade."
    });
});
...
app.MapGet("/api/auth/status", () =>
    Results.Ok(new ServiceStatus("Auth API", "v1", DateTimeOffset.UtcNow)))
    .Produces<ServiceStatus>()
    .RequireAuthorization();

app.MapPost("/api/auth/preview", (AuthRequest request) =>
    Results.Ok(new AuthResponse(request.ReferenceId, $"Auth API preview generated", DateTimeOffset.UtcNow)))
    .AddEndpointFilter(new ValidationFilter<AuthRequest>())
