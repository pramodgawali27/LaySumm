...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1", new OpenApiInfo
    {
        Title = "Audit API",
        Version = "v1",
        Description = "Audit logging and compliance evidence."
    });
});
...
app.MapGet("/api/audit/status", () =>
    Results.Ok(new ServiceStatus("Audit API", "v1", DateTimeOffset.UtcNow)))
    .Produces<ServiceStatus>()
    .RequireAuthorization();

app.MapPost("/api/audit/preview", (AuditRequest request) =>
    Results.Ok(new AuditResponse(request.ReferenceId, $"Audit API preview generated", DateTimeOffset.UtcNow)))
    .AddEndpointFilter(new ValidationFilter<AuditRequest>())
    .Produces<AuditResponse>()
