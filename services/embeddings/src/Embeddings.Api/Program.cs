...
using Serilog;
using Embeddings.Api.Contracts;
...
builder.Services.AddSwaggerGen(options =>
{
    options.SwaggerDoc("v1", new OpenApiInfo
    {
        ...
        Version = "v1",
        Description = "Vector embedding management service."
    });
});
...
app.MapGet("/api/embeddings/status", () =>
    Results.Ok(new ServiceStatus("Embeddings API", "v1", DateTimeOffset.UtcNow)))
    .Produces<ServiceStatus>()
    .RequireAuthorization();

app.MapPost("/api/embeddings/preview", (EmbeddingsRequest request) =>
    Results.Ok(new EmbeddingsResponse(request.ReferenceId, $"Embeddings API preview generated", DateTimeOffset.UtcNow)))
    .AddEndpointFilter(new ValidationFilter<EmbeddingsRequest>())
